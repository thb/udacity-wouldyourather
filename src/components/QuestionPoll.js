    ...
    <div>
      ...
      { [question.optionOne, question.optionTwo].map((option, index) => (
        <div key={index} className={`relative my-4 p-4 border-2 rounded ${option.votes.includes(authedUser) ? 'border-emerald-500 bg-emerald-100 text-emerald-600' : 'bg-gray-100'}`}>
          ...
          <div className="flex w-full bg-gray-200 rounded-md h-6 dark:bg-gray-300">
            <div className={`bg-emerald-500 h-6 rounded-md relative w-${option.votes.length}/${totalVotes}`}>
              <p className="text-white font-bold text-sm absolute right-2 mt-0.5">
                {percentage(option)}%
              </p>
            </div>
          </div>
          ...
          { option.votes.includes(authedUser) &&
            <div className="bg-yellow-400 text-white font-bold uppercase absolute -top-8 -right-8 mt-2 mr-2 text-xs rounded-full p-2">
              Your<br />vote
            </div>
          }
        </div>
